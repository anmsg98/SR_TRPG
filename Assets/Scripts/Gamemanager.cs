...
public class Gamemanager : MonoBehaviour
{
    ...
    void FindRoute(bool friend, GameObject Entity)
    {
        float distance = 99f;
        ...
        if (totalDistance > 1)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    Reset();
                    int cnt = BFS(currentPos.x + dx[i], currentPos.y + dy[i], targetPos.x, targetPos.y);
                    Debug.Log(cnt);
                    if (minDis >= cnt)
                    {
                        minDis = cnt;
    ...
    public void InsertMapInfo()
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                entityOrder.Add(camp[0].GetChild(j).gameObject);
            }
        }
    ...
    public IEnumerator FindRoutine()
    {
        for (int i = 0; i < 100; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                FindRoute(true, entityOrder[j]);
                yield return new WaitForSeconds(1f);
            }
            // for (int j = 3; j < 6; j++)
            // {
            //     FindRoute(false, entityOrder[j]);
            //     yield return new WaitForSeconds(1f);
            // }
        }
    }
        ...
        while (queue.Count != 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if ((dirx >= 0 && dirx < 5) && (diry >= 0 && diry < 10)
                                            && visited[dirx, diry] == false && map[dirx, diry] != 1) // 블루가 레드 때렸을때
                {
